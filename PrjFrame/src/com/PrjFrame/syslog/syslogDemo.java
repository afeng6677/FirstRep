...
public class syslogDemo {
	...
	public static void main(String[] args){
		try {
			...
			//设置syslog服务器端地址
			syslog.getConfig().setHost("192.168.1.110");
			//设置syslog接收端口，默认514
			syslog.getConfig().setPort(514);
			//拼接syslog日志，这个日志是自己定义的，通常我们定义成符合公司规范的格式就行，方便查询。例如 操作时间：2014年8月1日  操作者ID:张三 等。信息就是一个字符串。
			StringBuffer buffer = new StringBuffer();
			buffer.append("设备:" + "张三" + ";");
			buffer.append("时间:" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+ ";");
			buffer.append("类型:" + syslog.FACILITY_LOCAL0+ ";");
			 ...
			 */
//			syslog.emergency(URLDecoder.decode(buffer.toString(),"utf-8"));
			syslog.log(1, URLDecoder.decode(buffer.toString(),"utf-8"));
		} catch (Exception e) {
		}
	}
}
