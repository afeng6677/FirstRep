...
jQuery.TreeTable = function (tree_id, heads, tNodes) {
    if ($("#" + tree_id).length > 0) {
        ...
        for (var i = 0, l = newNodes.length; i < l; i++) {
            var node = newNodes[i];
            body_html += "<tr onclick=\"ck('"+node.id+"','"+node.name+"','"+node.pId+"')\" data-tt-id='" + node.id + "' " + (node.pId ? "data-tt-parent-id='" + node.pId + "'" : "") 
            + "><td class = 'td_left'><span class='" + (node.isParent ? "parent" : "children") + "'>" 
...
function changeColor(){
	var t = 0;
	$("#treeMenu tbody tr").each(function(i){
		if($(this).css("display")!="none"){
			++t;
			if(t%2==0){
				$(this).removeClass("singletr");
				$(this).addClass("doubletr");
			}else{
				$(this).removeClass("doubletr");
				$(this).addClass("singletr");
			}
		}
	});
	$("#treeMenu tbody tr").unbind("mouseover");
	$("#treeMenu tbody tr").mouseover(function(){
		$(this).addClass("active");
	});
	$("#treeMenu tbody tr").unbind("mouseout");
	$("#treeMenu tbody tr").mouseout(function(){
